...
如果依然无法使用, 建议优先排查浏览器依赖、OCR/LLM 配置与系统可用内存.
...
如果有其他您需要自定义的项目, 请通过[交流群](https://t.me/embykeeper_chat_bot)或 [Github Issues](https://github.com/emby-keeper/emby-keeper/issues) 反馈.
...
但是, 您需要注意不要在截图/反馈信息中带有个人隐私 (包括手机号 / Telegram UID / 用户名) 等, 建议使用马赛克遮挡后再发送到 [Github Issues](https://github.com/emby-keeper/emby-keeper/issues), 谢谢.
...
当前版本不再使用远程授权或远程验证码服务，所有签到请求都在本地执行。

部分带 CF 校验的站点签到失败，常见原因是：

1. 本地缺少过盾依赖（如 `playwright` Python 包，或 Chromium 浏览器运行环境）。
2. 当前网络或代理被站点风控拦截，即使拿到 token 也可能提交失败。
3. 站点自身策略调整，原有接口路径或参数发生变化。

如果日志提示能力缺失，站点会被直接跳过（`IGNORE`），不会再走远程兜底链路。

如果已经安装了 `playwright`，但仍然提示 `cf.turnstile` 能力缺失，通常还需要额外安装浏览器资源：

```bash
python -m playwright install chromium
```

若仍然失败，请继续检查当前环境是否允许无头 Chromium 启动，以及代理/网络是否能正常打开站点验证页。
