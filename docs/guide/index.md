...
Embykeeper 是一个专注于 Emby 影视服务器的自动化工具, 主要保留两项核心功能:

1. **自动签到** - 可以自动完成多个站点的 Telegram 机器人每日签到, 以获取积分.

2. **定时保号** - 通过模拟登录和播放视频, 定期保持 Emby 账号的活跃状态, 避免因长期不使用而被回收.

项目支持 Python 运行与 Docker 部署, 且完全开源, 不存储任何密钥或隐私信息, 经两年的开发已经在稳定和安全性方面有一定保证.

验证码识别、视觉题、文本推断和日志推送可以通过您自己的本地 LLM 与 Telegram Bot 完成.

::: tip 快速安装
希望安装 Embykeeper? 请前往 [**🚀 安装指南**](/guide/安装指南.md).

如果您使用 Debian 服务器, 推荐直接走 [**🌐 Debian Web 部署**](/guide/Debian-Web-部署.md).
:::
...
验证码识别、视觉题、文本推断和日志推送都可以改为本地 LLM 与您自己的 Telegram Bot 配置完成。
