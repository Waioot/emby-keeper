...
最后, 结束程序, 并将输出的 `log.json` 发送至 [讨论组](https://t.me/embykeeper_chat_bot) 或 [Github Issues](https://github.com/emby-keeper/emby-keeper/issues).
...
- `embykeeper.telegram.checkiner`
- `embykeeper.telegram.monitor`
- `embykeeper.telegram.messager`
...
通常来说, 增加一个机器人的签到非常简单, 您需要在 `checkiner` 中增加一个文件 `dummy.py`:
...
更多签到器配置参加: [BotCheckin 类定义](https://github.com/emby-keeper/emby-keeper/blob/main/embykeeper/telegram/checkiner/_base.py#L111) 和 [AnswerBotCheckin 类定义](https://github.com/emby-keeper/emby-keeper/blob/main/embykeeper/telegram/checkiner/_base.py#L560)
...
更多监控器配置参加: [Monitor 类定义](https://github.com/emby-keeper/emby-keeper/blob/main/embykeeper/telegram/monitor/_base.py#L100)
...
对于 `default_messages` 和对应的配置文件项目, 请直接参考当前 `messager` 模块中的实现与配置字段定义.

更多监控水群器配置参加: [Messager 类定义](https://github.com/emby-keeper/emby-keeper/blob/main/embykeeper/telegram/messager/_base.py#L49)
...
│   ├── settings.py             # 配置模块, 负责检查配置文件格式, 生成模板配置文件与准备配置文件.
│   ├── utils.py                # 一系列工具函数和类.
│   └── var.py                  # 一系列全局变量.
├── utils                       # 各测试工具.
│   └── ...
