...
from embykeeper.notifier.telegram_bot import (
    TelegramBotStream,
    resolve_bot_token,
    resolve_chat_id,
)
...
async def start_notifier():
    """消息通知初始化函数."""
    global stream_log, stream_msg, handler_log_id, handler_msg_id, change_handle_notifier, change_handle_bot
    ...
    if not notifier or not notifier.enabled:
        ...
        if getattr(config, "bot", None) and not change_handle_bot:
            change_handle_bot = config.on_change("bot", _handle_config_change)
        return None

    method = (notifier.method or "").strip().lower()
    if method == "apprise":
    ...
    if method != "telegram_bot":
        logger.error(f"未知的消息推送方式: {notifier.method}.")
        if not change_handle_notifier:
            change_handle_notifier = config.on_change("notifier", _handle_config_change)
        if getattr(config, "bot", None) and not change_handle_bot:
            change_handle_bot = config.on_change("bot", _handle_config_change)
    ...
    if not bot_token:
        logger.error("Telegram Bot Token 未配置, 无法发送消息推送.")
        return None
    if chat_id is None:
        logger.error("Telegram Bot 目标 chat_id 未配置, 无法发送消息推送.")
        return None

    logger.info("关键消息将通过 Telegram Bot 推送.")
    stream_log = TelegramBotStream(
        bot_token=bot_token,
        chat_id=chat_id,
        document_mode=True,
...
async def debug_notifier():
    ...
    if streams:
        ...
        method = (config.notifier.method or "").strip().lower()
        if method == "apprise":
